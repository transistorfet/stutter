...
int base_cmd_alias(char *env, char *args)
{
	...
	else if ((*name == '\0') || (&args[pos] == '\0') || !(str = create_string("%s", &args[pos])) || !(add_variable(NULL, type, name, 0, "callback,string", base_cmd_concat, str))) {
		BASE_ERROR_JOINPOINT(BASE_ERR_ALIAS_FAILED, name)
		return(-1);
	}
	BASE_COMMAND_RESPONSE_JOINPOINT(BASE_FMT_ALIAS, name, &args[pos])
	return(0);
}
